...
def create_toy_data(rng_key, N, d, k):
    ...
    #mus = jnp.array([-10. * jnp.ones(d), 10. * jnp.ones(d), -2. * jnp.ones(d), -2. * jnp.ones(d)])
    #sigs = jnp.reshape(jnp.array([0.1, 1., 0.1, 1.]), (4,1))
    
    #These are the tru posteriors because for acc comparison they use latent vals as original vals.
    mus=np.array([[-0.47513545, -0.64684845, -1.0844226,  -2.99815147], [-1.59160065,  2.63365976,  0.50422479 ,-0.16564698], [-0.74917041, 1.05338156, -0.85620659,  0.02077821],[ 2.16861619, -1.4764844,  -1.24448874,  1.84740613]])
    print('This is mus:', mus)
    sigs=np.array([[[ 9.37235615e-04,  2.25987751e-05,  4.61183927e-05,  1.42638076e-05],[ 2.25987751e-05,  9.50405006e-04,  1.62593922e-05,  2.06792498e-05],[ 4.61183927e-05,  1.62593922e-05,  9.39532406e-04,  3.20185322e-05],[ 1.42638076e-05,  2.06792498e-05,  3.20185322e-05,  9.40001157e-04]], [[ 1.00306828e-03, -5.14492393e-05,-1.14882290e-05, -6.31007307e-07],
    [-5.14492393e-05,  9.38380250e-04,  4.17939211e-05, -2.80141194e-05],[-1.14882290e-05,  4.17939211e-05,  9.95802822e-04,  1.19462795e-04],[-6.31007307e-07, -2.80141194e-05,  1.19462795e-04,  1.02342243e-03]],
    [[ 1.00395475e-03, -1.10737801e-05,  8.40664979e-06,  6.31296867e-05],[-1.10737801e-05,  9.68020873e-04,  1.58250408e-05, -5.18779824e-05],[ 8.40664979e-06,  1.58250408e-05,  9.86237799e-04,  7.73406299e-05],[ 6.31296867e-05, -5.18779824e-05,  7.73406299e-05,  1.06611216e-03]],
    [[ 1.10933542e-03,  2.76185777e-05,  2.81869156e-05, -3.25761644e-05],[ 2.76185777e-05,  1.14559295e-03,  2.80003862e-05,  2.05637248e-05], [ 2.81869156e-05,  2.80003862e-05,  1.13577963e-03, -6.51098259e-05],[-3.25761644e-05,  2.05637248e-05, -6.51098259e-05,  1.16274171e-03]]])
    print('This is sigs:', sigs)
    print('This is sigs.shape: ', sigs.shape)
    pis = jnp.array([1/4, 1/4, 1/4, 1/4])
    ...
    #z_train = z[:N]
    #print('This is z_train:', z_train)
    #print('This is z_train.shape:', z_train.shape)
    #X_train = X[:N]
    #z_test = z[N:]
    #X_test = X[N:]

    X_train=genfromtxt('mog_data_X_train.csv', delimiter=',')
    z_train=genfromtxt('mog_data_y_train.csv', delimiter=',')
    X_test=genfromtxt('mog_data_X_test.csv', delimiter=',')
    z_test=genfromtxt('mog_data_y_test.csv', delimiter=',')
...
def predict_assignment(
    ...
    return remapped_data_assignment
...
def main(args):
    ...
    X_train, X_test, latent_vals = create_toy_data(toy_data_rng, N, d, k)
    ...
    print("I want to know the params: ", params)
    ...
    testing_pred=predict_assignment(X_test, latent_vals[1], latent_vals[2], posterior_modes, posterior_pis
    )
    print('The predicted labels by dpvi:', testing_pred)
    print('The predicted labels by dpvi:', testing_pred.shape)
    ...
    print("This is sigs at the end (latent_vals[3]): ", latent_vals[3])

    print("This is svi_state: ", svi_state[0][1][0])
