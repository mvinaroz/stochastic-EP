...
def demo_clutter(seed, step, num_data, num_group, size, prior_precision, w,
		std_noise, learning_rate=0.1, c=10, clip=False, is_private=False, epsilon=1.0, delta=1e-6, mode='stochastic'):
	...
	for i in range(len(step)):
		clutter_train.train_ep(X, step[i], learning_rate, mode, noise_mean, noise_cov, c, clip=clip, \
			is_private=is_private)
		#y_pred, logZ_pred = clutter_train.predict(X_test, m_samp)
		#y_pred_train, _ = clutter_train.predict(X, m_samp)
		
		if i==(len(step)-1):
			#print("Computing averaged F-norm and averaged KL divergence")
...
def main():
	...
	kl_div_dp=np.zeros((len(epsilon), len(c)))
	...
	for k in range(len(epsilon)):
		...
		for i in range(ar.num_test):
			for j in range(len(c)):
				kl_div_dp1= demo_clutter(ar.seed[i], step, ar.num_data, ar.num_group, \
					ar.dimension, prior_precision, w, ar.std, c=c[j], clip=ar.clip, \
					learning_rate=learning_rate, is_private=ar.is_private, epsilon=epsilon[k], delta=ar.delta, mode='stochastic')
				#fnorm_mean_dp.append(err_mean_dp)
				#fnorm_cov_dp.append(err_cov_dp)
				kl_div_dp[k][j]=kl_div_dp1

				#Non-private clipped version
				#err_mean, err_cov, kl_div2= demo_clutter(ar.seed[i], step, ar.num_data, ar.num_group, \
				#	ar.dimension, prior_precision, w, ar.std, c=c[j], clip=ar.clip, \
				...
				#kl_div.append(kl_div2)

	kl_div_sep=demo_clutter(ar.seed[i], step, ar.num_data, ar.num_group, \
					ar.dimension, prior_precision, w, ar.std, c=c[j],  clip=False, \
					learning_rate=learning_rate, is_private=False, epsilon=1.0, delta=ar.delta, mode='stochastic')
	err_mean, err_cov=clutter_train.compute_mse(m_samp, var_samp)

	print("this is kl_div_sep: ", kl_div_sep)
	
	kl_div_ep=demo_clutter(ar.seed[i], step, ar.num_data, ar.num_group, \
                                        ar.dimension, prior_precision, w, ar.std, c=c[j],  clip=False, \
                                        learning_rate=learning_rate, is_private=False, epsilon=1.0, delta=ar.delta, mode='full')
	
	print("this is kl_div_ep ", kl_div_ep)
	...
	#kl_div_dp=np.array(kl_div)
	sep_kl=np.repeat(-kl_div_sep, len(c))
	ep_kl=np.repeat(-kl_div_ep, len(c))
	...
	for i in range(len(epsilon)):
		ax1.plot(c, kl_div_dp[i],  label="DP-SEP eps={}".format(epsilon[i]))
	ax1.plot(c, sep_kl,'o', label="SEP")
	ax1.plot(c, ep_kl, label="EP")
	ax1.set_xlabel('Clipping value C')
	ax1.set_ylabel('Averaged KL-divergence')
	#ax1.get_xaxis().set_ticks(c)
	...
	filename='averkl_num_data={}_learning_rate={}_num_iter={}_private={}_delta={}.pdf'.format(ar.num_data, \
			learning_rate, step.sum(), ar.is_private, ar.delta)
