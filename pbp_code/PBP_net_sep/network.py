...
class Network:
    ...
    def compute_cavity(self, prior_params, alpha, N, stochastic):
        ...
        for layer in self.layers:
            ...
            if stochastic:
                ...
                #print("This is the m_w for the cavity when stochastic=False: ", layer.m_w.get_value())
                m_w_c = m_w_q
                v_w_c = v_w_q
            m_w_n.append(m_w_c)
            v_w_n.append(v_w_c)
    ...
    def update_local(self, new_params, old_params, cavity_params_n, prior_params, alpha, N, stochastic):
        ...
        for layer in self.layers:
            ...
            # We compute the new approximation parameters (here is computing q_new = Nf_new + p_0 = q_old -1/N (q_old - p_0) +f_n)

            if stochastic:
                m_w_n_new_final = m_w_n_old - (m_w_n_old - m_w_p) / N + n_a * m_f_n #Add noise
                v_w_n_new_final = v_w_n_old - (v_w_n_old - v_w_p) / N + n_a * v_f_n #Add noise
                
            else:
                m_w_n_new_final = m_w_n_old + n_a * m_f_n
                v_w_n_new_final = v_w_n_old + n_a * v_f_n
            ...
            if len(index[ 0 ]) > 0:
                ...
                m_w_n_new_final[ index ] = m_w_n_old[ index ]
                v_w_n_new_final[ index ] = v_w_n_old[ index ]
