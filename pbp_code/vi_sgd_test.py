...
def generate_data(n, data_dim, n_hidden, lamb, gam):
    ...
    X_train=np.random.standard_normal(size=(n, data_dim))
    ...
    w_i_true=np.random.normal(0, size=(n, len_W0 + len_w1))*np.sqrt(1./lamb) + np.tile(w_true_mean, (n, 1))
    ...
    y_labels=np.random.standard_normal(n)*np.sqrt(1./gam) +  mean_y
    #print(y_labels)
