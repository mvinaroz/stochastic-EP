...
class PBP_net:
    ...
    def __init__(self, X_train, y_train, n_hidden, n_epochs = 40,
        ...
        print("WE ARE RUNNING PBP_net from sep folder!!!!!!")
        if normalize:
            self.std_X_train = np.std(X_train, 0)
            self.std_X_train[ self.std_X_train == 0 ] = 1
            self.mean_X_train = np.mean(X_train, 0)
            print("This is self.mean_X_train in __init__: ", self.mean_X_train)
        else:
        ...
        self.mean_y_train = 0 # np.mean(y_train)
        self.std_y_train = 1.0 # np.std(y_train)

        y_train_normalized = (y_train - self.mean_y_train) / self.std_y_train

        # We construct the network
        #print("This is X_train.shape[1]:", X_train.shape[1])

        #Check if we are doing probit regression (n_hidden=0) or PBP (n_hidden>0)
        if n_hidden[0]>0:
            print("We are doing PBP with {} hidden units".format(n_hidden))
            n_units_per_layer = \
        ...
        #print("This is n_units_per_layer: ", n_units_per_layer)

        self.pbp_instance = \
            pbp.PBP(n_units_per_layer, self.mean_y_train, self.std_y_train, fullEP)
    ...
    def predict(self, X_test):
        ...
        """
        print('This is self.mean_X_train: ', self.mean_X_train)
