...
class Network:
    ...
    def compute_cavity(self, i, alpha):
        ...
        for layer in self.layers:
            ...
            m_w_c = m_w_q - m_w_l / n_a
            v_w_c = v_w_q - v_w_l / n_a
            m_w_n.append(m_w_c)
            v_w_n.append(v_w_c)
            m_w_c = m_w_c / v_w_c
            v_w_c = 1.0 / v_w_c
        ...
        a_q = self.a.get_value() - 1	# natural param
        a_l = self.a_n[i]
        a_n = a_q - a_l / n_a
        a_c = a_n + 1
    ...
    def update_local(self, new_params, old_params, cavity_params_n, i, alpha):
        ...
        for layer in self.layers:
            ...
            # We compute the new site parameters

            m_w_n_site_new = (1 - step) * layer.m_w_n[ i ] + step * n_a * (m_w_n_new - m_w_n_cavity)
            v_w_n_site_new = (1 - step) * layer.v_w_n[ i ] + step * n_a * (v_w_n_new - v_w_n_cavity)

            # We compute the new approximation parameters

            m_w_n_new_final = m_w_n_old - layer.m_w_n[ i ] + m_w_n_site_new
            v_w_n_new_final = v_w_n_old - layer.v_w_n[ i ] + v_w_n_site_new
            ...
            if len(index[ 0 ]) > 0:
                ...
                #print("m_w_n_site_new shape: ", m_w_n_site_new)
                #print("This is m_w_n_site_new[ index ] before the modification: ", m_w_n_site_new[ index ])
                
                m_w_n_site_new[tuple(index)] = layer.m_w_n[ i ][tuple(index)]
                v_w_n_site_new[tuple(index)] = layer.v_w_n[ i ][tuple(index)]
                m_w_n_new_final[tuple(index)] = m_w_n_old[tuple(index)]
                v_w_n_new_final[tuple(index)] = v_w_n_old[tuple(index)]

            # We perform the update of the site parameters

            layer.m_w_n[ i ] = m_w_n_site_new
            layer.v_w_n[ i ] = v_w_n_site_new
        ...
        a_n_site_new = (1 - step) * self.a_n[ i ] + step * n_a * (a_n_new - a_n_cavity)
        b_n_site_new = (1 - step) * self.b_n[ i ] + step * n_a * (b_n_new - b_n_cavity)

        # We compute the new approximation parameters

        a_n_new_final = a_n_old - self.a_n[ i ] + a_n_site_new
        b_n_new_final = b_n_old - self.b_n[ i ] + b_n_site_new
        ...
        if np.isnan(a_n_new_final) or np.isnan(b_n_new_final) or a_n_new_final < 0 or b_n_new_final > 0 or \
            ...
            a_n_site_new = self.a_n[ i ]
            b_n_site_new = self.b_n[ i ]
            a_n_new_final = a_n_old
            b_n_new_final = b_n_old

        # We perform the update of the site parameters

        self.a_n[ i ] = a_n_site_new
        self.b_n[ i ] = b_n_site_new
