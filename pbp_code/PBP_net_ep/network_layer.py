...
class Network_layer:
    ...
    def __init__(self, m_w_init, v_w_init, non_linear = True, fullEP = None):
        ...
        # for full EP we need to initialise natural params for each site
        if fullEP is not None:
            # we use fullEP to store number of sites
            self.n_data = fullEP
            self.m_w_n = np.zeros((self.n_data, m_w_init.shape[0], m_w_init.shape[1]))
            self.v_w_n = np.zeros((self.n_data, v_w_init.shape[0], v_w_init.shape[1]))
